...
- 订单组件可承接报价转订单后的销售履约主档，但复杂订单必须先接客户、产品物料、报价和权限基座。
- `file-oss` 只提供订单附件、导出文件、物流文件的文件能力，不负责订单状态。
- 复杂订单接入前必须补正式 DDL、详情/退货/发货/关闭/导出/采购申请的数据范围守卫。
- `/delete`、`/confirmComplete` 等状态变更 GET 接口必须在目标项目收口为 DELETE/POST。
- 采购、仓储、财务是后续履约组件，本次装配只验证订单边界，不把全履约链路误判为已闭环。
