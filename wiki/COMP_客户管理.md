...
- 作为“客户 -> 报价 -> 订单”链路前置组件可用。
- 报价和订单选择客户时必须复用客户组件的数据范围，不能各自重写客户下拉。
- 复杂客户模型仍需目标项目补正式 DDL；简版项目可参考 legacy `customer`，但必须修正 `owner/follower` 缺列问题。
- 客户附件、头像、名片等文件能力建议接入 `file-oss`，不要在客户组件内独立实现上传。
