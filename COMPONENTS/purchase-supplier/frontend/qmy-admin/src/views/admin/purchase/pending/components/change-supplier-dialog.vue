<template>
  <el-dialog v-model="dialogVisible" title="更换供应商" width="400px" @close="onDestroy">
...
const { onDestroy, callback, selectedProducts, originalSupplier } = attrs as any
...
const loadSupplierList = async () => {
  const productSpecificationIdList = selectedProducts
    .filter(item => item.productSpecificationId)
    .map((item: any) => {
      return item.productSpecificationId
    })
  const ids = [...new Set(productSpecificationIdList)]
  const { code, data, message } = await listReplaceableSuppliers(ids)
  if (code !== 200) return ElMessage.warning(message)
