...
| 权限码 | 用途 | 来源状态 |
| :--- | :--- | :--- |
| `sal:yt:customer:updateAddress` | 新增/编辑客户地址 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:updateContactPerson` | 新增/编辑联系人 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:setAutoLevel` | 设置自动客户层级 | 后端启用 |
| `sal:yt:customer:enableStore` | 启用独立仓 | 后端启用 |
| `sal:yt:customer:auditStore` | 审核独立仓 | 后端启用 |
| `sal:yt:customer:addLabel` | 客户贴标签 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:deleteLabel` | 删除客户标签 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:follow` | 新增/编辑跟进记录 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:follow:delete` | 删除跟进记录 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:address:delete` | 删除地址 | 后端启用，已补客户范围校验 |
| `sal:yt:customer:contact:delete` | 删除联系人 | 后端启用，已补客户范围校验 |

## 数据权限

qmy-java 客户列表、详情、客户下拉、客户主档修改/删除、地址、联系人、标签和跟进写接口的源代码证据：
...
- 地址、联系人、标签、跟进表本身没有 `belong_employee_id` / `follow_employee_id` 字段；后端先解析子记录所属客户，再对 `sal_yt_customer` 执行同一客户数据范围校验。

## 正式接入要求

- 前端 `v-permission` 只控制按钮展示，不是安全边界。
- 所有修改、删除、标签、跟进、地址、联系人接口必须保留后端权限注解和客户范围校验。
