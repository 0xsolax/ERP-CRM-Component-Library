...
| P2 路由引用 `warehouse-history` 但快照缺文件 | 已补齐页面和业务类型常量 | `frontend/qmy-admin/src/views/admin/sales/warehouse-history/index.vue`、`frontend/qmy-admin/src/constant/yitang/warehouse.ts` |
| sed 客户页面引用 sed 客户 API | 已补齐客户组件自身 API 文件 | `frontend/qmy-admin/src/api/sed/sales/customer.ts` |
| sales 路由保留非客户页面引用 | 已裁剪订单/报价等非客户路由，避免组件快照引用未包含页面 | `frontend/qmy-admin/src/views/*/router/async-modules/sales.ts` |
...
| 非范围用户直接调用删除地址 | `GET /sal/yt/customer/address/delete?addressId=...&customerId=...`，地址所属客户不可见 | `selectById(customerId)` 在数据权限下不可见，返回“客户不存在或无权限” |
| 用户伪造 `customerId` 删除他人标签 | `GET /sal/yt/customer/deleteLabel?labelId=...&customerId=其他客户` | 后端解析标签真实 `masterId`，发现与请求 `customerId` 不一致并拒绝 |
...
- 已扫描前端路由引用，`warehouse-history/index.vue` 已在组件快照内，订单/报价等非客户路由已裁剪。
