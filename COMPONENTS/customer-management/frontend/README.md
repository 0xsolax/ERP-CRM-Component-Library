...
| `src/views/admin/sales/warehouse-history` | 一唐客户详情进入的独立仓历史流向页面 |
...
| `src/constant/*/customer.ts`、`src/constant/yitang/warehouse.ts` | 客户类型、手动层级和独立仓历史业务类型枚举 |
...
- 客户组件已包含自身路由直接引用的 `warehouse-history` 页面；`auth/org`、`system/region` 等仍由后台基座提供。
