...
import { ref, reactive, useAttrs, onMounted, computed } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { Plus, Minus, CircleClose } from '@element-plus/icons-vue'
import { imageFileType } from '@/constant/file-type'
import { createOrUpdateContactPerson } from '@/api/admin/sales/customer'
...
const hasContent = computed(() => {
  return !!(
    ...
    formData.socialPlatforms.some(i => i.account) ||
    formData.phones.some(i => i.number)
  )
})
...
const handleCardSuccess = (res: any) => {
  formData.cards.push({
    url: res.data.url,
...
const handleSubmit = async () => {
  ...
  if (fromType === 'customer-detail') {
    ...
    const { code, message } = await createOrUpdateContactPerson(apiData)
