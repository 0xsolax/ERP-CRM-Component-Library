...
import { createOrUpdateAddress } from '@/api/sed/sales/customer'
import { getCountries, getProvinces, getCitiesByProvinceId, getDistrictsByCityId } from '@/api/sed/system/region'
...
const cascaderProps = {
  ...
  lazyLoad: async (node: any, resolve: any) => {
    ...
    if (level === 0) {
      const { code, data } = await getProvinces()
      if (code === 200) {
      ...
      const { code, data } = await getCitiesByProvinceId({ provinceId: value })
      if (code === 200) {
        const nodes = (data || []).map((item: any) => ({
          value: item.regionId,
          label: item.regionName,
      ...
      const { code, data } = await getDistrictsByCityId({ cityId: value })
      if (code === 200) {
        const nodes = (data || []).map((item: any) => ({
          value: item.regionId,
          label: item.regionName,
