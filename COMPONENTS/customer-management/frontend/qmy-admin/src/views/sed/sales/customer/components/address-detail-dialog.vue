...
const displayConsignee = computed(() => {
  if (!addressData.value.consignee) return ''
  return showInfo.value ? addressData.value.consignee : desensitize(addressData.value.consignee, 1, 1)
})

const displayPhone = computed(() => {
  if (!addressData.value.phone) return ''
  return showInfo.value ? addressData.value.phone : desensitize(addressData.value.phone, 3, 4)
})

const displayAddress = computed(() => {
  if (!fullAddress.value) return ''
  return showInfo.value ? fullAddress.value : desensitize(fullAddress.value, 1, 1)
})
