...
import { router } from '@/views/zs/router'
...
const getStoredVisitedViews = (): TagView[] => {
  const stored = sessionStorage.getItem(ZsAdminKeys.visitedViewsKey)
  if (!stored) return []
...
const getStoredCachedViews = (): string[] => {
  const stored = sessionStorage.getItem(ZsAdminKeys.cachedViewsKey)
  if (!stored) return []
...
const saveVisitedViews = (views: TagView[]) => {
  ...
  sessionStorage.setItem(ZsAdminKeys.visitedViewsKey, JSON.stringify(serializedViews))
}

const saveCachedViews = (views: string[]) => {
  sessionStorage.setItem(ZsAdminKeys.cachedViewsKey, JSON.stringify(views))
}
