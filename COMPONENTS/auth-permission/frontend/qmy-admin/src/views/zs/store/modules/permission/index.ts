...
import { constantRoutes, asyncRoutes, router } from '@/views/zs/router'
...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const updateRoutesByPermissions = () => {
    ...
    state.routes = addRedirectRoute('zs', constantRoutes, filteredRoutes, router)
  ...
  const setRoutes = (routes: any[]) => {
    ...
    if (layoutSettings.showAdminAuthMenu) {
      state.routes = addRedirectRoute('zs', constantRoutes, accessedRoutes, router) // 路由菜单
      state.dynamicRoutes = flatRoutes(accessedRoutes) // 动态路由
    } else {
      state.routes = addRedirectRoute('zs', constantRoutes, asyncRoutes, router) // 本地路由菜单
