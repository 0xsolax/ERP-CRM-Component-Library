    ...
    <div class="login-box">
      ...
      <div v-else class="login-form-wrapper">
        <div class="login-header">
          <div class="login-title">欢迎登录，{{ tenantInfo.name }}</div>
          <div class="login-subtitle">{{ tenantInfo.shortLogo }}</div>
        </div>
        ...
        <div v-else class="feishu-login">
          <div class="feishu-title">{{ scanLoginTitle }}</div>
          <div class="qrcode-box">
            <div id="scan_qrcode_container" class="scan-qrcode-mount" />
            <div v-if="qrcodeLoading" class="qrcode-loading qrcode-overlay">
              <el-icon class="is-loading"><Loading /></el-icon>
              <div>加载中...</div>
            </div>
            <div v-else-if="qrcodeError" class="qrcode-error qrcode-overlay">
        ...
        <div class="other-login">
          ...
          <div class="other-login-icons">
            <div
              v-if="tenantInfo.accountSystemKey === 'FeiShu'"
              class="login-icon"
              :class="{ active: loginType === 'feishu' }"
              @click="switchLoginType('feishu')"
            >
            ...
            <div
              v-else-if="tenantInfo.accountSystemKey === 'DingDing'"
              class="login-icon"
              :class="{ active: loginType === 'dingding' }"
              @click="switchLoginType('dingding')"
            >
              <el-tooltip content="钉钉登录" placement="bottom">
                <img src="/images/login/dingding-icon.png" alt="" />
              </el-tooltip>
            </div>
...
import { reactive, ref, computed, toRefs, onMounted, onBeforeUnmount, nextTick } from 'vue'
import { useRouter } from 'vue-router'
import { useUserStore } from '@/views/zs/store/modules/user'
import { usePermissionStore } from '@/views/zs/store/modules/permission'
import { router as adminRouter } from '@/views/zs/router'
import { Loading, WarningFilled } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { getTenantDetail } from '@/api/zs/auth/user'
import { setZsAdminTenantInfo } from '@/utils/auth'
import { useFeishuLogin } from './hooks/use-feishu-login'
import { useDingdingLogin } from './hooks/use-dingding-login'

const ACCOUNT_SYSTEM_TO_SCAN_TYPE: Record<string, 'feishu' | 'dingtalk' | 'wecom'> = {
  FeiShu: 'feishu',
  DingDing: 'dingtalk',
  QiYeWeiXin: 'wecom'
}
...
const loginType = ref('') // feishu|dingding|account
const qrcodeLoading = ref(false)
const qrcodeError = ref('')
// const redirectUri = ref('http://test.zhongsheng.hzq.plus/zs/login')
const redirectUri = ref(window.location.origin + '/zs/login')
...
const scanLoginTitle = computed(() => {
  const key = tenantInfo.value.accountSystemKey
  if (key === 'DingDing') return '钉钉扫码登录'
  if (key === 'QiYeWeiXin') return '企业微信扫码登录'
  return '飞书扫码登录'
})
...
const switchLoginType = (type: string) => {
  ...
  if (type === 'dingding') {
    if (!tenantInfo.value.dingTalkAppKey) {
      return ElMessage.warning('当前租户未配置钉钉登录，请使用账号密码登录')
    }
  }
...
const loadTenantDetail = async () => {
  ...
  try {
    let domainName = window.location.hostname + '/zs'
    if (APP_ENV === 'development') {
      const proxyUrl = __APP_INFO__.proxy?.zsProxy || 'test.zhongsheng.hzq.plus'
      domainName = new URL(proxyUrl).hostname + '/zs'
      redirectUri.value = new URL(proxyUrl).origin + '/zs/login'
    }
    ...
    } else if (data?.accountSystemKey === 'DingDing') {
...
const { init: initFeishu, cleanup: cleanupFeishu } = useFeishuLogin({
  tenantInfo,
  redirectUri,
  qrcodeLoading,
  qrcodeError,
  onSuccess: code => handleScanLoginCallback(code, 'feishu')
})

const { init: initDingding } = useDingdingLogin({
  tenantInfo,
  redirectUri,
  qrcodeLoading,
  qrcodeError,
  onSuccess: code => handleScanLoginCallback(code, 'dingtalk')
})

const clearScanContainer = () => {
  const container = document.getElementById('scan_qrcode_container')
...
const initQrLogin = async () => {
  ...
  try {
    cleanupFeishu()
    clearScanContainer()
    const sysKey = tenantInfo.value.accountSystemKey
    if (sysKey === 'FeiShu') {
      initFeishu('scan_qrcode_container')
      return
    }
    if (sysKey === 'DingDing') {
      initDingding('scan_qrcode_container')
      return
    }
...
const handleScanLoginCallback = async (code: string, scanType: 'feishu' | 'dingtalk' | 'wecom') => {
  try {
    state.loading = true
    await userStore.loginByScan({ code, type: scanType })
...
onMounted(async () => {
  ...
  const scanType = ACCOUNT_SYSTEM_TO_SCAN_TYPE[tenantInfo.value.accountSystemKey]
  if (!scanType) return

  if (tenantInfo.value.accountSystemKey === 'FeiShu') {
    loginType.value = 'feishu'
  } else if (tenantInfo.value.accountSystemKey === 'DingDing') {
    loginType.value = 'dingding'
  }
  await handleScanLoginCallback(code, scanType)
})
