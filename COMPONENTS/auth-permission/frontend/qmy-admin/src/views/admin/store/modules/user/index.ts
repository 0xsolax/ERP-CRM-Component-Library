...
import {
  getYitangAdminToken,
  setYitangAdminToken,
  removeYitangAdminToken,
  setYitangAdminUserId,
  getYitangAdminUserId,
  removeYitangAdminUserId,
  setYitangAdminNickName,
  setYitangAdminUserName,
  removeYitangAdminUserName,
  removeYitangAdminNickName,
  getYitangAdminTenantInfo,
  removeYitangAdminTenantInfo
} from '@/utils/auth'
...
export const useUserStore = defineStore('user', () => {
  ...
  const state = reactive<UserState>({
    token: getYitangAdminToken() || '',
    userId: getYitangAdminUserId() || '',
    userName: '',
    nickName: '',
    avatar: '',
    // roleId: '',
    // roleName: '',
    loadUserInfo: false,
    tenantInfo: getYitangAdminTenantInfo() ?? {}
  })
  ...
  const login = async (payload: any) => {
    ...
    if (data?.token) {
      ...
      setYitangAdminToken(data?.token ?? '')
      setYitangAdminUserId(data?.userId ?? '')
      setYitangAdminNickName(data?.nickName ?? '')
      setYitangAdminUserName(data?.userName ?? '')
    }
  }
  ...
  const getUserInfo = async () => {
    ...
    state.avatar = data?.user?.avatar || '/images/avatar/default.png'
