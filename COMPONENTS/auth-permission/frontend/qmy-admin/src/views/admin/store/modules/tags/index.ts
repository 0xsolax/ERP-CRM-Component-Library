...
import { YitangAdminKeys } from '@/config/settings'
...
const getStoredVisitedViews = (): TagView[] => {
  const stored = sessionStorage.getItem(YitangAdminKeys.visitedViewsKey)
  if (!stored) return []
...
const getStoredCachedViews = (): string[] => {
  const stored = sessionStorage.getItem(YitangAdminKeys.cachedViewsKey)
  if (!stored) return []
...
const saveVisitedViews = (views: TagView[]) => {
  ...
  sessionStorage.setItem(YitangAdminKeys.visitedViewsKey, JSON.stringify(serializedViews))
}

const saveCachedViews = (views: string[]) => {
  sessionStorage.setItem(YitangAdminKeys.cachedViewsKey, JSON.stringify(views))
}
