...
import { QmyAdminKeys } from '@/config/settings'
import { router } from '@/views/qmy/router'
...
const getStoredVisitedViews = (): TagView[] => {
  const stored = sessionStorage.getItem(QmyAdminKeys.visitedViewsKey)
  if (!stored) return []
...
const getStoredCachedViews = (): string[] => {
  const stored = sessionStorage.getItem(QmyAdminKeys.cachedViewsKey)
  if (!stored) return []
...
const saveVisitedViews = (views: TagView[]) => {
  ...
  sessionStorage.setItem(QmyAdminKeys.visitedViewsKey, JSON.stringify(serializedViews))
}

const saveCachedViews = (views: string[]) => {
  sessionStorage.setItem(QmyAdminKeys.cachedViewsKey, JSON.stringify(views))
}
