...
import { usePermissionStore } from '@/views/sed/store/modules/permission'
import { router as adminRouter } from '@/views/sed/router'
import { Loading, WarningFilled } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { getTenantDetail } from '@/api/sed/auth/user'
import { setSedAdminTenantInfo } from '@/utils/auth'
...
// const redirectUri = ref('http://test.sed.hzq.plus/sed/login')
const redirectUri = ref(window.location.origin + '/sed/login')
...
const loadTenantDetail = async () => {
  ...
  try {
    let domainName = window.location.hostname + '/sed'
    if (APP_ENV === 'development') {
      const proxyUrl = __APP_INFO__.proxy?.sedProxy || 'test.sed.hzq.plus'
      domainName = new URL(proxyUrl).hostname + '/sed'
      redirectUri.value = new URL(proxyUrl).origin + '/sed/login'
    }
