package com.qmy.zhongsheng.core.base.manager.impl;

import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import com.qmy.zhongsheng.core.base.dao.BaseDataDAO;
...
public class BaseDataManagerImpl implements BaseDataManager {
    ...
    public List<BaseDataDO> listByNodeIds(List<Long> nodeIds) {
        return baseDataDAO.selectList(Wrappers.<BaseDataDO>lambdaQuery()
                .in(!CollectionUtils.isEmpty(nodeIds), BaseDataDO::getNodeId, nodeIds)
                .eq(BaseDataDO::getIsDeleted, 0)
    ...
    public List<BaseDataDO> listByNodeId(Long nodeId) {
        return baseDataDAO.selectList(Wrappers.<BaseDataDO>lambdaQuery()
                .eq(BaseDataDO::getNodeId, nodeId)
                .eq(BaseDataDO::getIsDeleted, 0)
                .orderByAsc(BaseDataDO::getId));
    }
    ...
    public BaseDataDO getByNodeIdAndValue1(Long nodeId, String value1) {
        return baseDataDAO.selectOne(Wrappers.<BaseDataDO>lambdaQuery()
                .eq(BaseDataDO::getNodeId, nodeId)
                .eq(BaseDataDO::getValue1, value1)
                .eq(BaseDataDO::getIsDeleted, 0)
                .last("LIMIT 1"));
    }

    @Override
    public BaseDataDO getByNodeIdAndValue1AndValue2(Long nodeId, String value1, String value2) {
        return baseDataDAO.selectOne(Wrappers.<BaseDataDO>lambdaQuery()
                .eq(BaseDataDO::getNodeId, nodeId)
                .eq(BaseDataDO::getValue1, value1)
                .eq(BaseDataDO::getValue2, value2)
                .eq(BaseDataDO::getIsDeleted, 0)
                .last("LIMIT 1"));
    }
    ...
    public Boolean deleted(Long id) {
        return baseDataDAO.update(Wrappers.<BaseDataDO>lambdaUpdate()
                .eq(BaseDataDO::getId, id)
                .set(BaseDataDO::getIsDeleted, 1)
                .set(BaseDataDO::getDeletedTime, LocalDateTime.now())) > 0;
    }
}
